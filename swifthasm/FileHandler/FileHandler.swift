...
public struct FileHandle {
    public static func open(
        ...
        if isBinary { mode += "b" }
        guard let handle = fopen(path.rawValue, mode) else { throw POSIXError.errno }
    ...
    public static func open<Result>(
        ...
        var file = try open(path, mode: mode, isBinary: isBinary)
        do {
    ...
    public func readLine(strippingNewline: Bool = false) throws -> String? {
        precondition(!isKnownInvalid, "Attempt to read an invalid file stream.")
    ...
    public func write<Bytes: Sequence>(_ bytes: Bytes) throws where Bytes.Element == UInt8 {
        ...
        if countWritten != count { throw POSIXError.errno }
    }
}
...
extension FileHandle: TextOutputStream {
    public func write(_ string: String) {
        precondition(!isKnownInvalid, "Attempt to write to an invalid file stream.")
        if fputs(string, handle) == EOF, let errorHandler = writeErrorHandler { errorHandler(.errno) }
    }
}
